---
bibliography: general_linear_hypothesis_application.bib
...
* $H_0: \mu_{x=l} - \frac{(\mu_{x=m} + \mu_{x = h})}{2} = \alpha - \frac{\alpha + \beta_1 + \alpha + \beta_2}{2} = \alpha - \frac{1}{2}\beta_1 - \frac{1}{2}\beta_2 = 0$ tests the null hypothesis that the mean value of $y$ in the referent category <i>low</i> ($x = l$) is the same as the mean value of $y$ with in either the <i>medium</i> or <i>high</i> categories ($x = m$ or $x =l$).

```{r otherEffectsTest, eval = TRUE, echo = TRUE, results = TRUE, warning = FALSE, message = FALSE}
oMat <- rbind(c(0, 0, -1), c(1, -1/2, -1/2))
oMat
...
Unless the goal is to publish in the Journal of Irreproducible Results, an analyst must adjust for simultaneously testing multiple hypotheses.
